...
#define TASK_SHARED_MEM_END 0x01400000
...
void task(void) {
  ...
  uintptr_t addr = 0x41414141;

  switch (val) {
  case 1:
    read_addr(TASK_SHARED_MEM_START - PAGE_SIZE);
    break;
  case 2:
    read_addr(TASK_SHARED_MEM_END);
    break;
  default:
    addr = 0x0 + PAGE_SIZE * val - 2 * PAGE_SIZE;
    if (addr >= TASK_SHARED_MEM_START) {
      while (true) {
